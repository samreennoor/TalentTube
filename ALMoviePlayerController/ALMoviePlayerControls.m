...
- (void)setup {
    ...
    _topBar.frame = CGRectMake(0,_bottomBar.frame.origin.y - _bottomBar.frame.size.height, _topBar.frame.size.width, _topBar.frame.size.height);
    ...
    UIImage *sliderLeftTrackImage = [[UIImage imageNamed: @"redbar.png"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 5, 0, 0)];
    UIImage *sliderRightTrackImage = [[UIImage imageNamed: @"whitebar.png"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 5, 0, 0)];
    UIImage *thumbImage = [UIImage imageNamed:@"slider-default7-handle.png"];
   
    [[UISlider appearance] setThumbImage:thumbImage forState:UIControlStateNormal];
    [_durationSlider setMinimumTrackImage: sliderLeftTrackImage forState: UIControlStateNormal];
    [_durationSlider setMaximumTrackImage: sliderRightTrackImage forState: UIControlStateNormal];
...
- (void)setBarImage:(UIImage *)barImage {
    if (_barImage != barImage) {
        _barImage = barImage;
        [self.topBar setColor:barImage];
        [self.bottomBar setColor:barImage];
    }
}
...
- (void)layoutSubviews {
    ...
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        ...
        if (IS_IPHONE_4) {
            self.topBar.frame = CGRectMake(0, 280, self.frame.size.width,50 /*self.barHeight*/);
         
        }else{
        self.topBar.frame = CGRectMake(0, 400, self.frame.size.width, self.barHeight);
        }
        ...
        self.playPauseButton.frame = CGRectMake(paddingFromBezel, self.barHeight/2 - fullscreenHeight/2 -5, fullscreenWidth, fullscreenHeight);
        ...
        //bottom bar
        if (IS_IPHONE_4) {
               self.bottomBar.frame = CGRectMake(0, self.frame.size.height - self.barHeight, self.frame.size.width, 40/* self.barHeight*/);
        }else{
      ...
      //  self.playPauseButton.frame = CGRectMake(self.bottomBar.frame.size.width/2 - playWidth/2, self.barHeight/2 - playHeight/2, playWidth, playHeight);
        ...
        self.bottomBar.frame = CGRectMake(0, self.frame.size.height - self.barHeight, self.frame.size.width, self.barHeight);
        self.topBar.frame = CGRectMake(0, 350, self.frame.size.width, self.barHeight);
