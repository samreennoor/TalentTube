...
- (id)initWithFrame:(CGRect)frame handleCharacterAllowed:(NSString *)handleCharacterAllowed hashtagCharacterAllowed:(NSString *)hashtagCharacterAllowed colorHandle:(UIColor *)colorHandle colorHashtag:(UIColor *)colorHashtag textToDisplay:(NSString *)textToDisplay delegate:(id<AutocompleteHandleTextViewDelegate>)instanceDelegates
{
    ...
    if (self)
    {
        ...
        [[self layer] setBorderColor:[[UIColor colorWithRed:0.667f green:0.667f blue:0.667f alpha:0.0f] CGColor]];
        [self setFont:[UIFont fontWithName:@"Montserrat" size:16.0f]];
...
-(void)autosizing
{
    ...
    CGSize newSize = [self sizeThatFits:CGSizeMake(fixedWidth, height)];
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, fixedWidth, height);
...
- (void)colorWord {
    ...
    NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc]initWithString:self.text];
     [attributedText setAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"Montserrat" size:16.0f], NSForegroundColorAttributeName:[UIColor darkGrayColor]} range:NSMakeRange(0, self.text.length)];
