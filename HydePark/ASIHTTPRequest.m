...
static const CFOptionFlags kNetworkArtsAndEntertainments =  kCFStreamEventHasBytesAvailable | kCFStreamEventEndEncountered | kCFStreamEventErrorOccurred;
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((ASIHTTPRequest*)clientCallBackInfo) handleNetworkArtsAndEntertainment: type];
}
...
- (void)startRequest
{
   ...
   // Start the HTTP connection
    ...
    if (CFReadStreamSetClient((CFReadStreamRef)[self readStream], kNetworkArtsAndEntertainments, ReadStreamClientCallBack, &ctxt)) {
...
- (void)handleNetworkArtsAndEntertainment:(CFStreamEventType)type
{	
...
- (void)scheduleReadStream
{
	if ([self readStream] && ![self readStreamIsScheduled]) {
		...
		[self setLastActivityTime:[NSDate date]];
		CFStreamClientContext ctxt = {0, self, NULL, NULL, NULL};
		CFReadStreamSetClient((CFReadStreamRef)[self readStream], kNetworkArtsAndEntertainments, ReadStreamClientCallBack, &ctxt);
